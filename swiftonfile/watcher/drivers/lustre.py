...
class LustreWatcher(GenericWatcher):
    ...
    async def fid_to_path(self, fid):
        ...
        if proc.returncode != 0:
            raise LustreError("Problem with fid2path: {}".format(stderr))
    ...
    async def get_lustre_default_data(
        self, container_path, subdirectory, filename, l_type
    ):
    ...
    async def get_lustre_rmdir_data(self, container_path, subdirectory, filename):
        ...
        try:
            all_files_to_delete = await self.list_container_files(
                container_path, path.join(subdirectory, filename)
            )
        except WatcherError:
            pass
    ...
    async def get_lustre_rename_data(
        ...
        filename_fullpath = path.join(
            self.node_folder, self.fs_name, container_path, subdirectory, filename
        )
        ...
        if path.isdir(filename_fullpath):
            ...
            file_to_add = await self.get_local_files_in_container(
                container_path, new_folder
            )
    ...
    async def get_changelog_data(self):
        ...
        for line in changelogs:
            ...
            if l_type == LUSTRE_TYPE_RMDIR:
                if container_path in all_containers_path:
                    await self.get_lustre_rmdir_data(
                        container_path, subdirectory, filename
                    )
                ...
                if container_path in all_containers_path:
                    await self.get_lustre_default_data(
                        container_path, subdirectory, filename, l_type
                    )
