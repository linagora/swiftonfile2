...
class TestConstraintsMiddleware(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_check = check_constraints.filter_factory(self.conf)(FakeApp())

    def test_GET(self):
        path = "/V1.0/a/c/o"
        resp = Request.blank(path, environ={"REQUEST_METHOD": "GET"}).get_response(
            self.test_check
        )
    ...
    def test_PUT_container(self):
        path = "/V1.0/a/c"
        resp = Request.blank(path, environ={"REQUEST_METHOD": "PUT"}).get_response(
            self.test_check
        )
        self.assertEqual(resp.status_int, 200)

    def test_PUT_container_bad_policy(self):
        path = "/V1.0/a/c"
        resp = Request.blank(
            path,
            environ={"REQUEST_METHOD": "PUT"},
            headers={"X-Storage-Policy": "bad_policy"},
    ...
    def test_PUT_bad_url(self):
        ...
        with mock.patch.object(check_constraints, "Request", request_mock):
            resp = Request.blank(path, environ={"REQUEST_METHOD": "PUT"}).get_response(
                self.test_check
            )
            self.assertEqual(resp.status_int, 200)
    ...
    def test_PUT_object_with_double_slashes(self):
        ...
        with nested(
            ...
            patch(
                "swiftonfile.swift.common.middleware.check_constraints." "POLICIES",
                self.policies_mock,
            ),
        ):
            resp = Request.blank(path, environ={"REQUEST_METHOD": "PUT"}).get_response(
            ...
            self.assertTrue(b"Invalid object name" in resp.body)
            self.assertTrue(b"cannot begin, end, or have" in resp.body)
    ...
    def test_PUT_object_end_with_slashes(self):
        ...
        with nested(
            ...
            self.assertTrue(b"Invalid object name" in resp.body)
            self.assertTrue(
                b"can end with a slash only if it is a directory" in resp.body
            )
    ...
    def test_PUT_object_named_dot(self):
        ...
        with nested(
            ...
            self.assertEqual(resp.status_int, 400)
            self.assertTrue(b"Invalid object name" in resp.body)
            self.assertTrue(b"cannot have . or .." in resp.body)

    def test_PUT_container_with_long_names(self):
        longname = "c" * 256
        path = "/V1.0/a/" + longname
        resp = Request.blank(
            path, method="PUT", headers={"X-Storage-Policy": "swiftonfile"}
        ).get_response(self.test_check)
        self.assertEqual(resp.status_int, 400)
    ...
    def test_PUT_object_with_long_names(self):
        ...
        with nested(
            ...
            self.assertEqual(resp.status_int, 400)
            self.assertTrue(b"too long" in resp.body)
