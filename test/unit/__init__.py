...
from swift.common.storage_policy import StoragePolicy, ECStoragePolicy, VALID_EC_TYPES
...
for eclib_name in EC_TYPE_PREFERENCE:
    ...
    raise SystemExit(
        "ERROR: unable to find suitable PyECLib type"
...
def patch_policies(
    thing_or_policies=None,
    legacy_only=False,
    with_ec_default=False,
    fake_ring_args=None,
):
    if isinstance(
        thing_or_policies, (Iterable, storage_policy.StoragePolicyCollection)
    ):
...
def write_fake_ring(path, *devs):
    ...
    """
    dev1 = {"id": 0, "zone": 0, "device": "sda1", "ip": "127.0.0.1", "port": 6200}
    dev2 = {"id": 1, "zone": 0, "device": "sdb1", "ip": "127.0.0.1", "port": 6200}
...
class FabricatedRing(Ring):
    ...
    def __init__(self, replicas=6, devices=8, nodes=4, port=6200, part_power=4):
    ...
    def _reload(self, *args, **kwargs):
        ...
        self._replica2part2dev_id = [
            [None] * 2**self.part_power for i in range(self.replicas)
        ]
...
if utils.config_true_value(get_config("unit_test").get("fake_syslog", "False")):
    fake_syslog_handler()
...
def mock_check_drive(isdir=False, ismount=False):
    ...
    with mocklib.patch(mock_base + "isdir") as mock_isdir, mocklib.patch(
        mock_base + "utils.ismount"
    ) as mock_ismount:
...
def fake_http_connect(*code_iter, **kwargs):
    class FakeConn(object):
        ...
        def getexpect(self):
            ...
            response = FakeConn(
                expect_status, timestamp=self.timestamp, headers=headers
            )
        ...
        def getheaders(self):
            ...
            if not etag:
                if isinstance(self.body, bytes):
                    etag = '"' + md5(self.body, usedforsecurity=False).hexdigest() + '"'
                else:
    ...
    if isinstance(kwargs.get("expect_headers"), (list, tuple)):
        ...
        expect_headers_iter = iter([kwargs.get("expect_headers", {})] * len(code_iter))
    ...
    with mocklib.patch("swift.common.bufferedhttp.http_connect_raw", new=fake_conn):
        ...
        if fake_conn.unexpected_requests:
            raise AssertionError(
                "unexpected requests:\n%s"
                % "\n  ".join("%r" % (req,) for req in fake_conn.unexpected_requests)
            )


def make_timestamp_iter(offset=0):
    return iter(Timestamp(t) for t in itertools.count(int(time.time()) + offset))
...
def mock_timestamp_now(now=None):
    if now is None:
        now = Timestamp.now()
    with mocklib.patch("swift.common.utils.Timestamp.now", classmethod(lambda c: now)):
        yield now
...
def encode_frag_archive_bodies(policy, body):
    ...
    fragment_payloads = []
    for chunk in chunks:
        fragments = policy.pyeclib_driver.encode(chunk) * policy.ec_duplication_factor
        if not fragments:
            break
        fragment_payloads.append(fragments)

    # join up the fragment payloads per node
    ec_archive_bodies = [b"".join(frags) for frags in zip(*fragment_payloads)]
    return ec_archive_bodies


def make_ec_object_stub(test_body, policy, timestamp):
    segment_size = policy.ec_segment_size
    test_body = test_body or (b"test" * segment_size)[: -random.randint(1, 1000)]
...
def fake_ec_node_response(node_frags, policy):
    ...
    def get_response(req):
        requested_policy = int(req["headers"]["X-Backend-Storage-Policy-Index"])
        if int(policy) != requested_policy:
            AssertionError("Requested polciy doesn't fit the fake response policy")
        if not node_map:
        ...
        try:
            ...
            raise Exception(
                "Couldn't find node %s:%s in %r" % (req["ip"], req["port"], all_nodes)
            )
        try:
        ...
        try:
            ...
            raise Exception(
                ...
                % (resp_frag["frag"], node_index, [f["frag"] for f in frags], err)
            )
        headers = {
            "X-Object-Sysmeta-Ec-Content-Length": len(obj_stub["body"]),
            "X-Object-Sysmeta-Ec-Etag": obj_stub["etag"],
            "X-Object-Sysmeta-Ec-Frag-Index": policy.get_backend_index(
                resp_frag["frag"]
            ),
            ...
            "X-Backend-Fragments": server._make_backend_fragments_header(ts2frags),
        }
...
def xattr_supported_check():
    ...
    try:
        ...
        if errno.errorcode.get(e.errno) in (
            "ENOSPC",
            "ENOTSUP",
            "EOPNOTSUPP",
            "ERANGE",
...
def skip_if_no_xattrs():
    if not xattr_supported_check():
        raise SkipTest(
            "Large xattrs not supported in `%s`. Skipping test" % gettempdir()
        )
        ...
        def replicate(self, op, *sync_args):
            ...
            if not resp:
                ...
                with mock_check_drive(
                    isdir=not rpc.mount_check, ismount=rpc.mount_check
                ):
...
def group_by_byte(contents):
    ...
    return [(char, sum(1 for _ in grp)) for char, grp in itertools.groupby(byte_iter)]
