...
class FakeLogger(logging.Logger, object):
    ...
    def _log(self, level, msg, *args, **kwargs):
        ...
        if "exc_info" in kwargs and not isinstance(kwargs["exc_info"], tuple):
    ...
    def get_lines_for_level(self, level):
        if level not in self.lines_dict:
            raise KeyError(
                ...
                % (level, ", ".join("'%s'" % lvl for lvl in sorted(self.lines_dict)))
            )
        return self.lines_dict[level]

    def all_log_lines(self):
        return dict(
            (level, msgs) for level, msgs in self.lines_dict.items() if len(msgs) > 0
        )
    ...
    def _handle(self, record):
        try:
            ...
            print(
                "WARNING: unable to format log message %r %% %r"
...
class DebugLogger(FakeLogger):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.formatter = DebugSwiftLogFormatter("%(server)s %(levelname)s: %(message)s")
