...
class Connection(object):
    def __init__(self, config):
        for key in "auth_uri username password".split():
            if key not in config:
                raise SkipTest("Missing required configuration parameter: %s" % key)
    ...
    def cluster_info(self):
        ...
        """
        status = self.make_request("GET", "/info", cfg={"absolute_path": True})
        if status // 100 == 4:
            return {}
    ...
    def http_connect(self):
        if (
            ...
            self.connection = self.conn_class(
                self.storage_netloc, context=ssl._create_unverified_context()
            )
        else:
    ...
    def make_path(self, path=None, cfg=None):
        ...
        if path:
            ...
            if cfg.get("no_quote") or cfg.get("no_path_quote"):
                quote = str_to_wsgi
            return "%s/%s" % (self.storage_path, "/".join([quote(i) for i in path]))
        else:
    ...
    def make_headers(self, hdrs, cfg=None):
        ...
        if isinstance(hdrs, dict):
            headers.update((str_to_wsgi(h), str_to_wsgi(v)) for h, v in hdrs.items())
        return headers

    def make_request(
        self, method, path=None, data=b"", hdrs=None, parms=None, cfg=None
    ):
        if path is None:
            path = []
        if hdrs is None:
    ...
    def request_with_retry(self, try_request):
        ...
        raise RequestError(
            "Attempts: %s, Failures: %s" % (len(fail_messages), fail_messages)
        )
...
class Base(object):
    ...
    def header_fields(self, required_fields, optional_fields=None):
        ...
        def is_int_header(header):
            if header.startswith("x-account-storage-policy-") and header.endswith(
                ("-bytes-used", "-object-count")
            ):
                return True
            return header in (
                "content-length",
        ...
        # on the wire
        headers = dict(
            (wsgi_to_str(h).lower(), wsgi_to_str(v))
        ...
        for return_key, header in required_fields:
            if header not in headers:
                raise ValueError(
                    "%s was not found in response headers: %r" % (header, headers)
                )
...
class Account(Base):
    ...
    def update_metadata(self, metadata=None, cfg=None):
        ...
        if cfg is None:
            cfg = {}
        headers = dict(("X-Account-Meta-%s" % k, v) for k, v in metadata.items())

        self.conn.make_request("POST", self.path, hdrs=headers, cfg=cfg)
        if not is_success(self.conn.response.status):
            raise ResponseError(
                self.conn.response, "POST", self.conn.make_path(self.path)
            )
        return True
    ...
    def containers(self, hdrs=None, parms=None, cfg=None):
        ...
        status = self.conn.make_request(
            "GET", self.path, hdrs=hdrs, parms=parms, cfg=cfg
        )
        if status == 200:
            if format_type == "json":
                ...
                for x in tree.getElementsByTagName("container"):
                    ...
                    for key in ["name", "count", "bytes", "last_modified"]:
                        cont[key] = (
                            x.getElementsByTagName(key)[0].childNodes[0].nodeValue
                        )
                    conts.append(cont)
                for cont in conts:
                    if six.PY2:
        ...
        raise ResponseError(self.conn.response, "GET", self.conn.make_path(self.path))

    def delete_containers(self):
        for c in listing_items(self.containers):
            cont = self.container(c)
            cont.update_metadata(
                hdrs={"x-versions-location": ""}, tolerate_missing=True
            )
    ...
    def info(self, hdrs=None, parms=None, cfg=None):
        ...
        if cfg is None:
            cfg = {}
        if (
            self.conn.make_request("HEAD", self.path, hdrs=hdrs, parms=parms, cfg=cfg)
            != 204
        ):
            raise ResponseError(
                self.conn.response, "HEAD", self.conn.make_path(self.path)
            )

        fields = [
            ["object_count", "x-account-object-count"],
...
class Container(Base):
    ...
    def create(self, hdrs=None, parms=None, cfg=None):
        ...
        return self.conn.make_request(
            "PUT", self.path, hdrs=hdrs, parms=parms, cfg=cfg
        ) in (201, 202)
    ...
    def update_metadata(self, hdrs=None, cfg=None, tolerate_missing=False):
        ...
        if tolerate_missing and self.conn.response.status == 404:
            return True
        raise ResponseError(self.conn.response, "POST", self.conn.make_path(self.path))
    ...
    def delete(self, hdrs=None, parms=None):
        ...
        if parms is None:
            parms = {}
        return (
            self.conn.make_request("DELETE", self.path, hdrs=hdrs, parms=parms) == 204
        )
    ...
    def files(self, hdrs=None, parms=None, cfg=None):
            ...
            if format_type == "json" or "versions" in parms:
                ...
                if six.PY2:
                    for file_item in files:
                        for key in ("name", "subdir", "content_type", "version_id"):
                            if key in file_item:
                ...
                for x in container.childNodes:
                    ...
                    if x.tagName == "object":
                        ...
                        raise ValueError("Found unexpected element %s" % x.tagName)
                    files.append(file_item)

                for file_item in files:
                    if "subdir" in file_item:
                        if six.PY2:
                            file_item["subdir"] = file_item["subdir"].encode("utf-8")
                    else:
                        if six.PY2:
                            file_item.update(
                                {
        ...
        raise ResponseError(
            self.conn.response, "GET", self.conn.make_path(self.path, cfg=cfg)
        )
    ...
    def info(self, hdrs=None, parms=None, cfg=None):
        ...
        if self.conn.response.status == 204:
            required_fields = [
                ["bytes_used", "x-container-bytes-used"],
        ...
        raise ResponseError(self.conn.response, "HEAD", self.conn.make_path(self.path))
...
class File(Base):
    ...
    def copy(
        self, dest_cont, dest_file, hdrs=None, parms=None, cfg=None, return_resp=False
    ):
        if hdrs is None:
            hdrs = {}
        if parms is None:
        ...
        if (
            self.conn.make_request(
                "COPY", self.path, hdrs=headers, cfg=cfg, parms=parms
            )
            != 201
        ):
            raise ResponseError(
                self.conn.response, "COPY", self.conn.make_path(self.path)
            )
        if return_resp:
    ...
    def copy_account(
        self, dest_account, dest_cont, dest_file, hdrs=None, parms=None, cfg=None
    ):
        if hdrs is None:
            hdrs = {}
        if parms is None:
        ...
        if "Destination-Account" in headers:
            headers["Destination-Account"] = urllib.parse.quote(
    ...
    def delete(self, hdrs=None, parms=None, cfg=None, tolerate_missing=False):
        ...
        if (
            self.conn.make_request("DELETE", self.path, hdrs=hdrs, cfg=cfg, parms=parms)
            not in allowed_statuses
        ):
            raise ResponseError(
                self.conn.response, "DELETE", self.conn.make_path(self.path)
            )
    ...
    def info(self, hdrs=None, parms=None, cfg=None):
        ...
        header_fields = self.header_fields(fields, optional_fields=optional_fields)
    ...
    def initialize(self, hdrs=None, parms=None):
        ...
        status = self.conn.make_request("HEAD", self.path, hdrs=hdrs, parms=parms)
        if status == 404:
            return False
    ...
    def read(
        ...
        status = self.conn.make_request(
            "GET", self.path, hdrs=hdrs, cfg=cfg, parms=parms
        )

        if not is_success(status):
            raise ResponseError(
                self.conn.response, "GET", self.conn.make_path(self.path)
            )
    ...
    def sync_metadata(self, metadata=None, cfg=None, parms=None):
        ...
        if self.metadata:
            ...
            self.conn.make_request(
                "POST", self.path, hdrs=headers, parms=parms, cfg=cfg
            )
    ...
    def chunked_write(self, data=None, hdrs=None, parms=None, cfg=None):
        ...
        if data is not None and self.chunked_write_in_progress:
            ...
            self.conn.put_start(
                self.path, hdrs=headers, parms=parms, cfg=cfg, chunked=True
            )
    ...
    def write(
        ...
        if not is_success(self.response.status):
            raise ResponseError(
                self.conn.response, "PUT", self.conn.make_path(self.path)
            )
    ...
    def write_random_return_resp(self, size=None, hdrs=None, parms=None, cfg=None):
        if hdrs is None:
            hdrs = {}
        if parms is None:
            parms = {}
        ...
        data = self.random_data(size)
        resp = self.write(data, hdrs=hdrs, parms=parms, cfg=cfg, return_resp=True)
        if not resp:
    ...
    def post(self, hdrs=None, parms=None, cfg=None, return_resp=False):
        ...
        self.conn.make_request("POST", self.path, hdrs=headers, parms=parms, cfg=cfg)
