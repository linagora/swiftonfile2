...
from test.functional.swift_test_client import (
    Account,
    Connection,
    File,
    ResponseError,
...
class Utils(object):
    ...
    def create_utf8_name(cls, length=None):
        ...
        ustr = "".join([random.choice(utf8_chars) for x in range(length)])
        if six.PY2:
...
class Base(unittest.TestCase):
    ...
    def assert_body(self, body):
        ...
        self.assertEqual(response_body, body, "Body returned: %s" % (response_body))

    def assert_status(self, status_or_statuses):
        self.assertTrue(
            self.env.conn.response.status == status_or_statuses
            or (
                hasattr(status_or_statuses, "__iter__")
                and self.env.conn.response.status in status_or_statuses
            ),
    ...
    def assert_header(self, header_name, expected_value):
        try:
            ...
            self.fail("Expected header name %r not found in response." % header_name)
...
class TestAccount(Base):
    ...
    def testNoAuthToken(self):
        self.assertRaises(
            ResponseError, self.env.account.info, cfg={"no_auth_token": True}
        )
        self.assert_status([401, 412])

        self.assertRaises(
            ResponseError, self.env.account.containers, cfg={"no_auth_token": True}
        )
        self.assert_status([401, 412])
    ...
    def testVersionOnlyPath(self):
        self.env.account.conn.make_request("PUT", cfg={"version_only_path": True})
        self.assert_status(412)
    ...
    def testContainerSerializedInfo(self):
        ...
        for format_type in ["json", "xml"]:
            for a in self.env.account.containers(parms={"format": format_type}):
                self.assertGreaterEqual(a["count"], 0)
                self.assertGreaterEqual(a["bytes"], 0)

            headers = dict(
                (k.lower(), v) for k, v in self.env.conn.response.getheaders()
            )
            if format_type == "json":
                self.assertEqual(
    ...
    def testListingLimit(self):
        ...
        for lim in (1, 100, limit / 2, limit - 1, limit, limit + 1, limit * 2):
            ...
            if lim <= limit:
                self.assertLessEqual(len(self.env.account.containers(parms=p)), lim)
                self.assert_status(200)
            else:
                self.assertRaises(ResponseError, self.env.account.containers, parms=p)
                self.assert_status(412)
    ...
    def testListDelimiter(self):
        ...
        results = self.env.account.containers(
            parms={"delimiter": delimiter, "reverse": "yes"}
        )
        expected.reverse()
        results = [r for r in results if r in expected]
        self.assertEqual(expected, results)
    ...
    def testListDelimiterAndPrefix(self):
        ...
        results = self.env.account.containers(
            parms={"delimiter": delimiter, "prefix": "ba"}
        )
        expected = ["bar", "baza"]
        results = [r for r in results if r in expected]
        self.assertEqual(expected, results)
        ...
        results = self.env.account.containers(
            parms={"delimiter": delimiter, "prefix": "ba", "reverse": "yes"}
        )
        expected.reverse()
        results = [r for r in results if r in expected]
        self.assertEqual(expected, results)
    ...
    def testContainerListingLastModified(self):
        ...
        for container in self.env.containers:
            res = container.info()
            expected[container.name] = time.mktime(parsedate(res["last_modified"]))

        for format_type in ["json", "xml"]:
            actual = {}
            containers = self.env.account.containers(parms={"format": format_type})
            if isinstance(containers[0], dict):
                for container in containers:
                    ...
                    datetime_str, micro_sec_str = container["last_modified"].split(".")
                    timestamp = time.mktime(
                        time.strptime(datetime_str, "%Y-%m-%dT%H:%M:%S")
                    )
    ...
    def testLastContainerMarker(self):
        for format_type in [None, "json", "xml"]:
            ...
            marker = containers[-1] if format_type is None else containers[-1]["name"]
    ...
    def testMarkerLimitContainerList(self):
        for format_type in [None, "json", "xml"]:
            for marker in [
                ...
                containers = self.env.account.containers(
                    parms={"format": format_type, "marker": marker, "limit": limit}
                )
                self.assertLessEqual(len(containers), limit)
                if containers:
                    if isinstance(containers[0], dict):
                        containers = [x["name"] for x in containers]
                    self.assertGreater(locale.strcoll(containers[0], marker), 0)
    ...
    def testContainersOrderedByName(self):
        for format_type in [None, "json", "xml"]:
            ...
            if isinstance(containers[0], dict):
                containers = [x["name"] for x in containers]
            self.assertEqual(sorted(containers, key=locale.strxfrm), containers)
...
class TestAccountNoContainers(Base):
    def testGetRequest(self):
        for format_type in [None, "json", "xml"]:
            self.assertFalse(self.env.account.containers(parms={"format": format_type}))
...
class TestAccountSorting(Base):
    ...
    def testAccountContainerListSorting(self):
        ...
        for reverse in ("false", "no", "off", "", "garbage"):
            cont_listing = self.env.account.containers(parms={"reverse": reverse})
            self.assert_status(200)
            self.assertEqual(
                cont_list,
                cont_listing,
    ...
    def testAccountContainerListSortingByMarkersExclusive(self):
        ...
        cont_list = sorted(c for c in self.env.cont_items if last_item < c < first_item)
        cont_list.reverse()
        cont_listing = self.env.account.containers(
            parms={"reverse": "on", "marker": first_item, "end_marker": last_item}
        )
        self.assert_status(200)
        self.assertEqual(cont_list, cont_listing)
    ...
    def testAccountContainerListSortingByMarkersInclusive(self):
        ...
        cont_list = sorted(
            c for c in self.env.cont_items if last_item <= c <= first_item
        )
        cont_list.reverse()
        cont_listing = self.env.account.containers(
            parms={
                "reverse": "on",
                "marker": first_item + "\x00",
    ...
    def testAccountContainerListSortingByReversedMarkers(self):
        cont_listing = self.env.account.containers(
            parms={"reverse": "on", "marker": "B", "end_marker": "b1"}
        )
        self.assert_status(204)
...
class TestContainer(Base):
    ...
    def testPrefixAndLimit(self):
        ...
        for format_type in [None, "json", "xml"]:
            for prefix in prefixs:
                files = cont.files(parms={"prefix": prefix, "format": format_type})
    ...
    def testListDelimiter(self):
        ...
        for format_type in [None, "json", "xml"]:
            ...
            results = cont.files(parms={"delimiter": delimiter, "format": format_type})
            if isinstance(results[0], dict):
                results = [x.get("name", x.get("subdir")) for x in results]
            self.assertEqual(results, ["test", "test-"])

            results = cont.files(
                parms={"delimiter": delimiter, "format": format_type, "reverse": "yes"}
            )
            if isinstance(results[0], dict):
                results = [x.get("name", x.get("subdir")) for x in results]
            self.assertEqual(results, ["test-", "test"])
    ...
    def testListMultiCharDelimiter(self):
        ...
        for format_type in [None, "json", "xml"]:
            ...
            if isinstance(results[0], dict):
                results = [x.get("name", x.get("subdir")) for x in results]
            self.assertEqual(results, ["test", "test-&bar", "test-&foo", "test-'baz"])
    ...
    def testLeadingDelimiter(self):
        ...
        try:
            ...
            self.fail("ResponseError waited")
    ...
    def testContainerFileListOnContainerThatDoesNotExist(self):
        for format_type in [None, "json", "xml"]:
            ...
            self.assertRaises(
                ResponseError, container.files, parms={"format": format_type}
            )
            self.assert_status(404)
    ...
    def testUtf8Container(self):
        ...
        self.assertFalse(container.create(cfg={"no_path_quote": True}))
        self.assert_status(412)
        self.assertRaises(ResponseError, container.files, cfg={"no_path_quote": True})
        self.assert_status(412)
    ...
    def testSlashInName(self):
        ...
        self.assertFalse(
            cont.create(cfg={"no_path_quote": True}),
    ...
    def testFileCreateInContainerThatDoesNotExist(self):
        file_item = File(
            self.env.conn, self.env.account, Utils.create_name(), Utils.create_name()
        )
    ...
    def _testContainerFormattedFileList(self, format_type):
        ...
        for actual in file_list:
            ...
            if tf.cluster_info.get("etag_quoter", {}).get("enable_by_default"):
                self.assertEqual(expected[name]["etag"], '"%s"' % actual["hash"])
            else:
                self.assertEqual(expected[name]["etag"], actual["hash"])
            self.assertEqual(expected[name]["content_type"], actual["content_type"])
            self.assertEqual(expected[name]["content_length"], actual["bytes"])
            expected.pop(name)
    ...
    def testContainerInfo(self):
        ...
        self.assertEqual(info["bytes_used"], self.env.file_count * self.env.file_size)
    ...
    def testContainerFileListWithLimit(self):
        for format_type in [None, "json", "xml"]:
            files = self.env.container.files(parms={"format": format_type, "limit": 2})
    ...
    def testContainerLastModified(self):
        ...
        self.assertTrue(container.update_metadata({"x-container-meta-japan": "mitaka"}))
        info = container.info()
        ...
        self.assertTrue(obj.write(b"aaaaa", hdrs={"Content-Type": "text/plain"}))
        info = container.info()
        t3 = info["last_modified"]
        self.assertEqual(t2, t3)

        # POST object also doesn't change container last modified timestamp
        self.assertTrue(obj.sync_metadata({"us": "austin"}))
        info = container.info()
...
class TestContainerSorting(Base):
    ...
    def testContainerFileSortingByPrefixReversed(self):
        ...
        cont_list.reverse()
        cont_listing = self.env.container.files(parms={"reverse": "on", "prefix": "a"})
        self.assert_status(200)
        self.assertEqual(cont_list, cont_listing)
    ...
    def testContainerFileSortingByMarkersExclusiveReversed(self):
        ...
        cont_list = sorted(c for c in self.env.file_items if last_item < c < first_item)
        cont_list.reverse()
        cont_listing = self.env.container.files(
            parms={"reverse": "on", "marker": first_item, "end_marker": last_item}
        )
        self.assert_status(200)
        self.assertEqual(cont_list, cont_listing)
    ...
    def testContainerFileSortingByMarkersInclusiveReversed(self):
        ...
        cont_list = sorted(
            c for c in self.env.file_items if last_item <= c <= first_item
        )
        cont_list.reverse()
        cont_listing = self.env.container.files(
            parms={
...
class TestFile(Base):
    ...
    def testGetResponseHeaders(self):
        ...
        def do_test(put_hdrs, get_hdrs, expected_hdrs, unexpected_hdrs):
            ...
            resp = file_item.write(data=obj_data, hdrs=put_hdrs, return_resp=True)
            ...
            for k, v in expected_hdrs.items():
                ...
                for got_k, got_v in matches:
                    ...
                    if k == "connection":
                        got_v = got_v.lower()
                    if got_v != v:
                        errors.append("Expected %s but got %s for %s" % (v, got_v, k))
            if errors:
                self.fail("Errors in response headers:\n  %s" % "\n  ".join(errors))
        ...
        expected_headers = {
            "content-length": str(len(obj_data)),
            "x-object-meta-fruit": "Banana",
            "accept-ranges": "bytes",
            "content-type": "application/test",
    ...
    def testCopy(self):
        ...
        for cont in (self.env.container, dest_cont):
            ...
            for prefix in ("", "/"):
                ...
                self.assertEqual(data, file_copy.read())
                self.assertTrue(file_copy.initialize())
                self.assertEqual("application/test-updated", file_copy.content_type)
                expected_metadata = {"extra": "fresher"}
                self.assertDictEqual(expected_metadata, file_copy.metadata)
                resp_headers = dict(file_copy.conn.response.getheaders())
                ...
                self.assertEqual(file_item.size, obj["bytes"])
                self.assertEqual(normalize_etag(file_item.etag), obj["hash"])
                self.assertEqual("application/test-updated", obj["content_type"])
    ...
    def testCopyAccount(self):
        ...
        for cont in (self.env.container, dest_cont):
            ...
            for prefix in ("", "/"):
                ...
                file_item = self.env.container.file(source_filename)
                file_item.copy_account(acct, "%s%s" % (prefix, cont), dest_filename)
        ...
        if not tf.skip2:
            ...
            self.assertTrue(
                dest_cont.create(hdrs={"X-Container-Write": self.env.conn.user_acl})
            )
            ...
            for prefix in ("", "/"):
                ...
                file_item.copy_account(
                    acct, "%s%s" % (prefix, dest_cont), dest_filename
                )
    ...
    def testCopyAccount404s(self):
        ...
        dest_cont = self.env.account.container(Utils.create_name())
        self.assertTrue(
            dest_cont.create(hdrs={"X-Container-Read": self.env.conn2.user_acl})
        )
    ...
    def testCopyFromAccountHeader(self):
        ...
        self.assertTrue(
            src_cont.create(hdrs={"X-Container-Read": self.env.conn2.user_acl})
        )
        source_filename = Utils.create_name()
        ...
        dest_cont2 = self.env.account2.container(Utils.create_name())
        self.assertTrue(
            dest_cont2.create(hdrs={"X-Container-Write": self.env.conn.user_acl})
        )
            ...
            for prefix in ("", "/"):
                ...
                file_item.write(
                    hdrs={
                        ...
                        "X-Copy-From": "%s%s/%s"
                        % (prefix, src_cont.name, source_filename),
                    }
                )
    ...
    def testCopyFromHeader404s(self):
        ...
        for prefix in ("", "/"):
            # invalid source container
            file_item = self.env.container.file(Utils.create_name())
            copy_from = "%s%s/%s" % (prefix, Utils.create_name(), source_filename)
            self.assertRaises(
                ResponseError, file_item.write, hdrs={"X-Copy-From": copy_from}
            )
            self.assert_status(404)
            ...
            copy_from = "%s%s/%s" % (prefix, self.env.container.name, source_filename)
            self.assertRaises(
                ResponseError, file_item.write, hdrs={"X-Copy-From": copy_from}
            )
            self.assert_status(404)
    ...
    def testCopyFromAccountHeader404s(self):
        ...
        self.assertTrue(
            src_cont.create(hdrs={"X-Container-Read": self.env.conn.user_acl})
        )
        source_filename = Utils.create_name()
        file_item = src_cont.file(source_filename)
        file_item.write_random()
        ...
        for prefix in ("", "/"):
            ...
            self.assertRaises(
                ...
                hdrs={
                    "X-Copy-From-Account": acct,
                    "X-Copy-From": "%s%s/%s" % (prefix, src_cont, Utils.create_name()),
                },
            )
            self.assert_status(404)
            ...
            self.assertRaises(
                ...
                hdrs={
                    "X-Copy-From-Account": acct,
                    "X-Copy-From": "%s%s/%s" % (prefix, src_cont, source_filename),
                },
            )
            self.assert_status(404)
            ...
            while len(metadata.keys()) < i:
                ...
                if len(key) > j:
                    ...
                    if six.PY2:
                        ...
                        val = val.encode("utf8")[:j].decode("utf8", "surrogateescape")
    ...
    def testRangedGets(self):
        ...
        for i in range(0, file_length, range_size):
            ...
            self.assertEqual(
                data[i : i + range_size], file_item.read(hdrs=hdrs), range_string
            )
            ...
            if i == 0:
                ...
                self.assert_header(
                    "content-range",
                    "bytes %d-%d/%d" % (file_length - i, file_length - 1, file_length),
                )
            ...
            hdrs = {"Range": range_string}
            self.assertEqual(
                file_item.read(hdrs=hdrs), data[i - file_length :], range_string
            )
    ...
    def testMultiRangeGets(self):
        ...
        file_item = self.env.container.file(Utils.create_name())
        data = file_item.write_random(
            file_length, hdrs={"Content-Type": "lovecraft/rugose; squamous=true"}
        )
        ...
        for i in range(0, file_length, range_size):
            ...
            self.assertEqual(
                byteranges[0].get_payload(decode=True), data[i : (i + subrange_size)]
            )
            ...
            self.assertEqual(
                ...
                "bytes %d-%d/%d"
                % (i + 2 * subrange_size, i + 3 * subrange_size - 1, file_length),
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                "bytes %d-%d/%d"
                % (i + 4 * subrange_size, i + 5 * subrange_size - 1, file_length),
            )
            self.assertEqual(
        ...
        self.assertEqual(byteranges[0].get_payload(decode=True), data[:subrange_size])
        ...
        self.assertEqual(
            byteranges[1]["Content-Range"],
            "bytes %d-%d/%d" % (2 * subrange_size, 3 * subrange_size - 1, file_length),
        )
        self.assertEqual(
            byteranges[1].get_payload(decode=True),
    ...
    def testFileSizeLimit(self):
        ...
        for i in (
            ...
            if i <= limit:
                self.assertTrue(
                    timeout(tsecs, file_item.write, cfg={"set_content_length": i})
                )
            else:
                self.assertRaises(
                    ResponseError,
    ...
    def testNoContentLengthForPut(self):
        file_item = self.env.container.file(Utils.create_name())
        self.assertRaises(
            ResponseError, file_item.write, b"testing", cfg={"no_content_length": True}
        )
    ...
    def testBadHeaders(self):
        ...
        # bad range headers
        self.assertEqual(
            len(file_item.read(hdrs={"Range": "parsecs=8-12"})), file_length
        )
        self.assert_status(200)
    ...
    def testMetadataLengthLimits(self):
        ...
        lengths = [
            ...
            [key_limit, value_limit + 1],
            [key_limit + 1, value_limit],
            [key_limit, 0],
            [key_limit, value_limit * 10],
            [key_limit * 10, value_limit],
        ]

        for l in lengths:  # noqa
    ...
    def testSerialization(self):
        ...
        for format_type in ["json", "xml"]:
            for file_item in container.files(parms={"format": format_type}):
                ...
                for f in files:
                    ...
                    self.assertEqual(file_item["content_type"], f["content_type"])
                    self.assertEqual(int(file_item["bytes"]), f["bytes"])

                    d = datetime.strptime(
                        file_item["last_modified"].split(".")[0], "%Y-%m-%dT%H:%M:%S"
                    )
            ...
            headers = dict(
                (h.lower(), v) for h, v in self.env.conn.response.getheaders()
            )
            if format_type == "json":
                self.assertEqual(
                    headers["content-type"], "application/json; charset=utf-8"
        ...
        lm_diff = max([f["last_modified"] for f in files]) - min(
            [f["last_modified"] for f in files]
        )
        self.assertLess(
            lm_diff,
            write_time + 1,
            "Diff in last modified times " "should be less than time to write files",
        )
        ...
        for f in files:
            for format_type in ["json", "xml"]:
                self.assertTrue(
                    f[format_type],
                    "File %s not found in %s listing" % (f["name"], format_type),
                )
    ...
    def testChunkedPut(self):
        if tf.web_front_end == "apache2":
            raise SkipTest("Chunked PUT cannot be tested with apache2 web " "front end")
    ...
    def test_POST(self):
        ...
        self.assertEqual(1024, file_item.size)
        # SwiftOnFile: Assert not working
        # TODO: Fix SOF to make this test works
...
class TestFileComparisonEnv(BaseEnv):
    ...
    def setUp(cls):
        ...
        cls.time_old_f1 = time.strftime(
            "%a, %d %b %Y %H:%M:%S GMT", time.gmtime(time.time() - 86400)
        )
        cls.time_old_f2 = time.strftime(
            "%A, %d-%b-%y %H:%M:%S GMT", time.gmtime(time.time() - 86400)
        )
        cls.time_old_f3 = time.strftime(
            "%a %b %d %H:%M:%S %Y", time.gmtime(time.time() - 86400)
        )
        cls.time_new = time.strftime(
            "%a, %d %b %Y %H:%M:%S GMT", time.gmtime(time.time() + 86400)
        )
...
class TestFileComparison(Base):
    ...
    def testIfNoneMatchMultipleEtags(self):
        for file_item in self.env.files:
            ...
            hdrs = {"If-None-Match": '"bogus1", "bogus2", "%s"' % file_item.md5}
            self.assertRaises(ResponseError, file_item.read, hdrs=hdrs)
            self.assert_status(304)
            self.assert_etag(file_item.md5)
            self.assert_header("accept-ranges", "bytes")
    ...
    def testIfMatchAndUnmodified(self):
        for file_item in self.env.files:
            hdrs = {"If-Match": file_item.md5, "If-Unmodified-Since": self.env.time_new}
            self.assertTrue(file_item.read(hdrs=hdrs))

            hdrs = {"If-Match": "bogus", "If-Unmodified-Since": self.env.time_new}
            self.assertRaises(ResponseError, file_item.read, hdrs=hdrs)
            self.assert_status(412)
            self.assert_etag(file_item.md5)

            hdrs = {
                "If-Match": file_item.md5,
...
class TestServiceToken(unittest.TestCase):
    ...
    def _assert_is_authed_response(self, method, container, object, resp):
        ...
        self.assertTrue(
            expect(resp.status),
            "Unexpected %s for %s %s %s" % (resp.status, method, container, object),
        )
    ...
    def do_request(self, url, token, parsed, conn, service_token=""):
        ...
        if self.dbg:
            print(
                "DEBUG: conn.request: method:%s path:%s"
                " body:%s headers:%s" % (self.method, path, self.body, headers)
            )
    ...
    def test_user_cannot_access_service_account(self):
        for method, container, obj in self._scenario_generator():
            self.prepare_request(
                method, use_service_account=True, container=container, obj=obj
            )
            resp = retry(self.do_request)
