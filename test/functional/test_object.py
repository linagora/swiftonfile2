...
class TestObject(unittest.TestCase):
    ...
    def test_metadata(self):
        ...
        def put(url, token, parsed, conn):
            ...
            conn.request(
                "PUT", "%s/%s/%s" % (parsed.path, self.container, obj), "", headers
            )
            return check_response(conn)
        ...
        def post(url, token, parsed, conn):
            ...
            headers.update(req_metadata)
            conn.request(
                "POST", "%s/%s/%s" % (parsed.path, self.container, obj), "", headers
            )
            return check_response(conn)
        ...
        self.assertEqual(b"", resp.read())
        self.assertEqual(resp.status, 200)
        self.assertEqual(metadata(resp), {"X-Object-Meta-Color": "oraNge"})
    ...
    def test_too_small_x_timestamp(self):
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/%s" % (parsed.path, self.container, "too_small_x_timestamp"),
                "",
                {"X-Auth-Token": token, "Content-Length": "0", "X-Timestamp": "-1"},
            )
            return check_response(conn)
        ...
        if resp.status == 400:
            ...
            self.assertIn("X-Timestamp should be a UNIX timestamp float value", body)
        else:
            self.assertEqual(resp.status, 201)
    ...
    def test_too_big_x_timestamp(self):
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/%s" % (parsed.path, self.container, "too_big_x_timestamp"),
                "",
                {
                    "X-Auth-Token": token,
                    "Content-Length": "0",
    ...
    def test_x_delete_after(self):
        def put(url, token, parsed, conn):
            conn.request(
                ...
                "",
                {"X-Auth-Token": token, "Content-Length": "0", "X-Delete-After": "2"},
            )
            return check_response(conn)
    ...
    def test_non_integer_x_delete_after(self):
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/%s"
                % (parsed.path, self.container, "non_integer_x_delete_after"),
                "",
                {"X-Auth-Token": token, "Content-Length": "0", "X-Delete-After": "*"},
            )
            return check_response(conn)
    ...
    def test_non_integer_x_delete_at(self):
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/%s" % (parsed.path, self.container, "non_integer_x_delete_at"),
                "",
                {"X-Auth-Token": token, "Content-Length": "0", "X-Delete-At": "*"},
            )
            return check_response(conn)
    ...
    def test_x_delete_at_in_the_past(self):
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/%s" % (parsed.path, self.container, "x_delete_at_in_the_past"),
                "",
                {"X-Auth-Token": token, "Content-Length": "0", "X-Delete-At": "0"},
            )
            return check_response(conn)
    ...
    def test_copy_object(self):
        ...
        def get_source(url, token, parsed, conn):
            conn.request(
                "GET", "%s/%s" % (parsed.path, source), "", {"X-Auth-Token": token}
            )
            return check_response(conn)
        ...
        def get_dest(url, token, parsed, conn):
            conn.request(
                "GET", "%s/%s" % (parsed.path, dest), "", {"X-Auth-Token": token}
            )
            return check_response(conn)
        ...
        def delete(url, token, parsed, conn):
            conn.request(
                "DELETE", "%s/%s" % (parsed.path, dest), "", {"X-Auth-Token": token}
            )
            return check_response(conn)
        ...
        def copy(url, token, parsed, conn):
            conn.request(
                ...
                "",
                {"X-Auth-Token": token, "Destination": dest},
            )
            return check_response(conn)
        ...
        def copy(url, token, parsed, conn):
            conn.request(
                ...
                "%s/%s" % (parsed.path, source),
                "",
                {"X-Auth-Token": token, "Destination": dest, "Range": "bytes=1-2"},
            )
            return check_response(conn)
    ...
    def test_copy_between_accounts(self):
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                "",
                {"X-Auth-Token": token, "X-Container-Read": tf.swift_test_perm[1]},
            )
            return check_response(conn)
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                "%s/%s" % (parsed.path, self.container),
                "",
                {"X-Auth-Token": token, "X-Container-Write": tf.swift_test_perm[0]},
            )
            return check_response(conn)
    ...
    def test_public_object(self):
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                "",
                {"X-Auth-Token": token, "X-Container-Read": ".r:*"},
            )
            return check_response(conn)
    ...
    def test_container_write_only(self):
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                "%s/%s/%s" % (parsed.path, shared_container, "obj1"),
                "",
                {"X-Auth-Token": token, "X-Object-Meta-Color": "blue"},
            )
            return check_response(conn)
    ...
    def test_manifest(self):
        ...
        def put(url, token, parsed, conn, objnum):
            conn.request(
                "PUT",
                "%s/%s/segments1/%s" % (parsed.path, self.container, str(objnum)),
                segments1[objnum],
                {"X-Auth-Token": token},
            )
            return check_response(conn)
        ...
        def put(url, token, parsed, conn, objnum):
            conn.request(
                "PUT",
                "%s/%s/segments2/%s" % (parsed.path, self.container, str(objnum)),
                segments2[objnum],
                {"X-Auth-Token": token},
            )
            return check_response(conn)
        ...
        def put(url, token, parsed, conn):
            conn.request(
                "PUT", parsed.path + "/" + acontainer, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_null_name(self):
        ...
        def put(url, token, parsed, conn):
            conn.request(
                "PUT",
                "%s/%s/abc%%00def" % (parsed.path, self.container),
                "test",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
    ...
    def test_cors(self):
        ...
        def check_cors(url, token, parsed, conn, method, obj, headers):
            if method != "OPTIONS":
                headers["X-Auth-Token"] = token
            conn.request(
                method, "%s/%s/%s" % (parsed.path, self.container, obj), "", headers
            )
        ...
        resp = retry(check_cors, "OPTIONS", "cat", {"Origin": "http://m.com"})
        ...
        resp = retry(
            ...
            "OPTIONS",
            "cat",
            {"Origin": "http://m.com", "Access-Control-Request-Method": "GET"},
        )
        ...
        resp = retry(check_cors, "GET", "cat", {"Origin": "http://m.com"})
        self.assertEqual(resp.status, 200)
        headers = dict((k.lower(), v) for k, v in resp.getheaders())
        self.assertEqual(headers.get("access-control-allow-origin"), "*")

        resp = retry(
            check_cors, "GET", "cat", {"Origin": "http://m.com", "X-Web-Mode": "True"}
        )
        self.assertEqual(resp.status, 200)
        headers = dict((k.lower(), v) for k, v in resp.getheaders())
        self.assertEqual(headers.get("access-control-allow-origin"), "*")
        ...
        if strict_cors:
            ...
            resp = retry(check_cors, "GET", "cat", {"Origin": "http://secret.com"})
            resp.read()
            self.assertEqual(resp.status, 200)
            headers = dict((k.lower(), v) for k, v in resp.getheaders())
            self.assertEqual(
                headers.get("access-control-allow-origin"), "http://secret.com"
            )
        else:
            resp = retry(check_cors, "GET", "cat", {"Origin": "http://m.com"})
            resp.read()
            self.assertEqual(resp.status, 200)
            headers = dict((k.lower(), v) for k, v in resp.getheaders())
            self.assertEqual(headers.get("access-control-allow-origin"), "http://m.com")

    @requires_policies
    def test_cross_policy_copy(self):
        # create container in first policy
        policy = self.policies.select()
        container = self._create_container(headers={"X-Storage-Policy": policy["name"]})
        obj = uuid4().hex
    ...
    def test_bulk_delete(self):
        def bulk_delete(url, token, parsed, conn):
            ...
            conn.request(
                "DELETE",
                "%s/%s/%s?bulk-delete" % (parsed.path, self.container, self.obj),
        ...
        errors = [
            c.data if c.nodeType == c.TEXT_NODE else c.childNodes[0].data
