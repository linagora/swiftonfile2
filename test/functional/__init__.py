...
skip, skip2, skip3, skip_if_not_v3, skip_service_tokens, skip_if_no_reseller_admin = (
    False,
    False,
    False,
    False,
...
def _in_process_find_conf_file(conf_src_dir, conf_file_name, use_sample=True):
    ...
    """
    dflt_src_dir = os.path.normpath(
        os.path.join(os.path.abspath(__file__), os.pardir, os.pardir, os.pardir, "etc")
    )
...
def _in_process_setup_ring(swift_conf, conf_src_dir, testdir):
    ...
    if policy_specified:
        ...
        if policy_to_test is None:
            raise InProcessException(
                'Failed to find policy name "%s"' % policy_specified
            )
    ...
    try:
        ring_file_src = _in_process_find_conf_file(
            conf_src_dir, ring_file_src, use_sample=False
        )
...
def _load_encryption(proxy_conf_file, swift_conf_file, **kwargs):
    ...
    try:
        ...
        pipeline = pipeline.replace(
            "cache listing_formats", "cache etag-quoter listing_formats"
        )
        ...
        msg = "Error problem with proxy conf file %s: %s" % (proxy_conf_file, err)
        raise InProcessException(msg)
...
def _load_domain_remap_staticweb(proxy_conf_file, swift_conf_file, **kwargs):
    ...
    try:
        ...
        pipeline = old_pipeline.replace(" tempauth ", " tempauth staticweb ")
        pipeline = pipeline.replace(
...
def _load_s3api(proxy_conf_file, swift_conf_file, **kwargs):
    ...
    try:
        section = "pipeline:main"
        pipeline = conf.get(section, "pipeline")
        pipeline = pipeline.replace("tempauth", "s3api tempauth")
        conf.set(section, "pipeline", pipeline)
...
def in_process_setup(the_object_server=object_server):
    ...
    proxy_conf, swift_conf = _load_domain_remap_staticweb(proxy_conf, swift_conf)
    ...
    if conf_loader_label is not None:
        try:
            ...
            _debug(
                "Calling method %s mapped to conf loader %s"
                % (conf_loader.__name__, conf_loader_label)
            )
        except KeyError as missing_key:
            raise InProcessException(
                "No function mapped for conf loader %s" % missing_key
            )
    ...
    with mock.patch("swift.common.utils.get_logger", get_logger):
        with mock.patch(
            "swift.common.middleware.memcache.MemcacheMiddleware",
    ...
    global proxy_srv
    proxy_srv = prolis
    prospa = eventlet.spawn(
        eventlet.wsgi.server, prolis, app, nl, protocol=SwiftHttpProtocol
    )
    acc1spa = eventlet.spawn(
        eventlet.wsgi.server, acc1lis, acc1srv, nl, protocol=SwiftHttpProtocol
    )
    acc2spa = eventlet.spawn(
        eventlet.wsgi.server, acc2lis, acc2srv, nl, protocol=SwiftHttpProtocol
    )
    con1spa = eventlet.spawn(
        eventlet.wsgi.server, con1lis, con1srv, nl, protocol=SwiftHttpProtocol
    )
    con2spa = eventlet.spawn(
        eventlet.wsgi.server, con2lis, con2srv, nl, protocol=SwiftHttpProtocol
    )

    objspa = [
        eventlet.spawn(
            eventlet.wsgi.server, objsrv[0], objsrv[1], nl, protocol=SwiftHttpProtocol
        )
...
def get_cluster_info():
    ...
    for k in constraints.DEFAULT_CONSTRAINTS:
        try:
            ...
            print(
                "Invalid constraint value: %s = %s" % (k, test_constraints[k]),
                file=sys.stderr,
            )
...
def setup_package():
    ...
    if in_process:
        ...
        try:
            in_process_setup(
                the_object_server=(mem_object_server if in_mem_obj else object_server)
            )
        except InProcessException as exc:
            print(("Exception during in-process setup: %s" % str(exc)), file=sys.stderr)
            raise
    else:
        skip_if_no_xattrs = lambda: None  # noqa
    ...
    if config:
        ...
        if "auth_uri" in config:
            ...
            config.setdefault(
                "s3_storage_url", urlunsplit(parsed[:2] + ("", None, None))
            )
            ...
            config.setdefault(
                "s3_storage_url", urlunsplit((scheme, netloc, "", None, None))
            )
    ...
    if not skip_if_not_v3:
        skip_if_not_v3 = swift_test_auth_version != "3" or not all(
            [not skip, swift_test_user[3], swift_test_key[3]]
        )
    ...
    if not skip_service_tokens:
        ...
        if not skip and skip_service_tokens:
            print(
                "SKIPPING FUNCTIONAL TESTS SPECIFIC TO SERVICE TOKENS", file=sys.stderr
            )
    ...
    if not skip_if_no_reseller_admin:
        skip_if_no_reseller_admin = not all(
            [not skip, swift_test_user[5], swift_test_key[5], swift_test_tenant[5]]
        )
...
def reset_acl():
    def post(url, token, parsed, conn):
        conn.request(
            ...
            parsed.path,
            "",
            {"X-Auth-Token": token, "X-Account-Access-Control": "{}"},
        )
...
class FunctionalStoragePolicyCollection(object):
    ...
    def __init__(self, policies):
        ...
        for p in self:
            if p.get("default", False):
                ...
                ), "Found multiple default " "policies %r and %r" % (self.default, p)
                self.default = p
    ...
    def from_info(cls, info=None):
        ...
        assert policies.default, "Did not find default policy in %r" % policy_info
        return policies
    ...
    def filter(self, **kwargs):
        return self.__class__(
            [p for p in self if all(p.get(k) == v for k, v in kwargs.items())]
        )
