...
class TestContainer(unittest.TestCase):
    def setUp(self):
        ...
        def put(url, token, parsed, conn):
            conn.request(
                "PUT", parsed.path + "/" + self.name, "", {"X-Auth-Token": token}
            )
    ...
    def tearDown(self):
        ...
        def get(url, token, parsed, conn, container):
            conn.request(
                "GET",
                parsed.path + "/" + container + "?format=json",
                "",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
        ...
        def delete(url, token, parsed, conn, container):
            conn.request(
                "DELETE", parsed.path + "/" + container, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_multi_metadata(self):
        ...
        def head(url, token, parsed, conn):
            conn.request(
                "HEAD", parsed.path + "/" + self.name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_unicode_metadata(self):
        ...
        if six.PY2:
            self.assertEqual(
                resp.getheader("X-Container-Meta-uni"), uni_value.encode("utf-8")
            )
        else:
            self.assertEqual(resp.getheader("X-Container-Meta-uni"), uni_value)
    ...
    def test_PUT_metadata(self):
        ...
        def head(url, token, parsed, conn, name):
            conn.request("HEAD", parsed.path + "/" + name, "", {"X-Auth-Token": token})
            return check_response(conn)

        def get(url, token, parsed, conn, name):
            conn.request("GET", parsed.path + "/" + name, "", {"X-Auth-Token": token})
            return check_response(conn)

        def delete(url, token, parsed, conn, name):
            conn.request(
                "DELETE", parsed.path + "/" + name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_POST_metadata(self):
        ...
        def get(url, token, parsed, conn):
            conn.request(
                "GET", parsed.path + "/" + self.name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_PUT_bad_metadata(self):
        ...
        name = uuid4().hex
        resp = retry(
            put, name, {"X-Container-Meta-" + ("k" * self.max_meta_name_length): "v"}
        )
        resp.read()
        self.assertIn(resp.status, (201, 202))
        resp = retry(delete, name)
        ...
        resp = retry(
            put,
            name,
            {"X-Container-Meta-" + ("k" * (self.max_meta_name_length + 1)): "v"},
        )
        resp.read()
        ...
        name = uuid4().hex
        resp = retry(
            put, name, {"X-Container-Meta-Too-Long": "k" * self.max_meta_value_length}
        )
        resp.read()
        self.assertIn(resp.status, (201, 202))
        resp = retry(delete, name)
        ...
        resp = retry(
            put,
            name,
            {"X-Container-Meta-Too-Long": "k" * (self.max_meta_value_length + 1)},
        )
        resp.read()
        self.assertEqual(resp.status, 400)
        resp = retry(delete, name)
        ...
        if self.max_meta_overall_size - size > 1:
            headers["X-Container-Meta-k"] = "v" * (
                self.max_meta_overall_size - size - 1
            )
        resp = retry(put, name, headers)
        resp.read()
        self.assertIn(resp.status, (201, 202))
        ...
        self.assertIn(resp.status, (204, 404))
        name = uuid4().hex
        headers["X-Container-Meta-k"] = "v" * (self.max_meta_overall_size - size)
        resp = retry(put, name, headers)
        resp.read()
        self.assertEqual(resp.status, 400)
        resp = retry(delete, name)
    ...
    def test_POST_bad_metadata(self):
        ...
        resp = retry(
            post, {"X-Container-Meta-" + ("k" * self.max_meta_name_length): "v"}
        )
        resp.read()
        self.assertEqual(resp.status, 204)
        resp = retry(
            post, {"X-Container-Meta-" + ("k" * (self.max_meta_name_length + 1)): "v"}
        )
        resp.read()
        self.assertEqual(resp.status, 400)

        resp = retry(
            post, {"X-Container-Meta-Too-Long": "k" * self.max_meta_value_length}
        )
        resp.read()
        self.assertEqual(resp.status, 204)
        resp = retry(
            post, {"X-Container-Meta-Too-Long": "k" * (self.max_meta_value_length + 1)}
        )
        resp.read()
        self.assertEqual(resp.status, 400)
    ...
    def test_POST_bad_metadata3(self):
        ...
        headers["X-Container-Meta-k"] = "x" * (self.max_meta_overall_size - size)
        resp = retry(post, headers)
        resp.read()
        self.assertEqual(resp.status, 400)
        # this POST would be ok and the aggregate backend metadata
        # size is on the border
        headers = {"X-Container-Meta-k": "y" * (self.max_meta_overall_size - size - 1)}
        resp = retry(post, headers)
        resp.read()
        self.assertEqual(resp.status, 204)
        # this last POST would be ok by itself but takes the aggregate
        # backend metadata size over limit
        headers = {"X-Container-Meta-k": "z" * (self.max_meta_overall_size - size)}
        resp = retry(post, headers)
        resp.read()
        self.assertEqual(resp.status, 400)
    ...
    def test_public_container(self):
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                parsed.path + "/" + self.name,
                "",
                {"X-Auth-Token": token, "X-Container-Read": ".r:*,.rlistings"},
            )
            return check_response(conn)
    ...
    def test_cross_account_container(self):
        ...
        def get2(url, token, parsed, conn):
            conn.request(
                "GET", first_account[0] + "/" + self.name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_cross_account_public_container(self):
        ...
        def put2(url, token, parsed, conn):
            conn.request(
                "PUT",
                first_account[0] + "/" + self.name + "/object",
                "test object",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                parsed.path + "/" + self.name,
                "",
                {"X-Auth-Token": token, "X-Container-Write": tf.swift_test_perm[1]},
            )
            return check_response(conn)
    ...
    def test_nonadmin_user(self):
        ...
        def post(url, token, parsed, conn):
            conn.request(
                ...
                parsed.path + "/" + self.name,
                "",
                {"X-Auth-Token": token, "X-Container-Write": tf.swift_test_perm[2]},
            )
            return check_response(conn)
    ...
    def test_read_only_acl_listings(self):
        ...
        def put(url, token, parsed, conn, name):
            conn.request("PUT", parsed.path + "/%s" % name, "", {"X-Auth-Token": token})
            return check_response(conn)
    ...
    def test_read_only_acl_metadata(self):
        ...
        def get(url, token, parsed, conn, name):
            conn.request("GET", parsed.path + "/%s" % name, "", {"X-Auth-Token": token})
            return check_response(conn)
    ...
    def test_read_write_acl_listings(self):
        ...
        def delete(url, token, parsed, conn, name):
            conn.request(
                "DELETE", parsed.path + "/%s" % name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_long_name_content_type(self):
        ...
        def put(url, token, parsed, conn):
            ...
            conn.request(
                "PUT",
                "%s/%s" % (parsed.path, container_name),
                "there",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
    ...
    def test_null_name(self):
        ...
        def put(url, token, parsed, conn):
            conn.request(
                "PUT", "%s/abc%%00def" % parsed.path, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def test_create_container_gets_default_policy_by_default(self):
        try:
            default_policy = tf.FunctionalStoragePolicyCollection.from_info().default
        ...
        def head(url, token, parsed, conn):
            conn.request(
                "HEAD", parsed.path + "/" + self.container, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
        ...
        self.assertEqual(headers.get("x-storage-policy"), default_policy["name"])

    def test_error_invalid_storage_policy_name(self):
        def put(url, token, parsed, conn, headers):
            new_headers = dict({"X-Auth-Token": token}, **headers)
            conn.request("PUT", parsed.path + "/" + self.container, "", new_headers)
            return check_response(conn)
    ...
    def test_create_non_default_storage_policy_container(self):
        ...
        def put(url, token, parsed, conn, headers=None):
            ...
            conn.request("PUT", parsed.path + "/" + self.container, "", base_headers)
            return check_response(conn)
        ...
        resp.read()
        headers = dict((k.lower(), v) for k, v in resp.getheaders())
        self.assertEqual(headers.get("x-storage-policy"), policy["name"])
    ...
    def test_noop_change_storage_policy_with_post(self):
        ...
        def post(url, token, parsed, conn, headers):
            new_headers = dict({"X-Auth-Token": token}, **headers)
            conn.request("POST", parsed.path + "/" + self.container, "", new_headers)
            return check_response(conn)
...
class BaseTestContainerACLs(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        def get(url, token, parsed, conn):
            conn.request(
                "GET",
                parsed.path + "/" + self.name + "?format=json",
                "",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
        ...
        def delete(url, token, parsed, conn):
            conn.request(
                "DELETE", parsed.path + "/" + self.name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
    ...
    def _assert_cross_account_acl_granted(self, granted, grantee_account, acl):
        ...
        def get2(url, token, parsed, conn):
            conn.request(
                "GET", first_account + "/" + self.name, "", {"X-Auth-Token": token}
            )
            return check_response(conn)
        ...
        def put2(url, token, parsed, conn):
            conn.request(
                "PUT",
                first_account + "/" + self.name + "/object",
                "test object",
                {"X-Auth-Token": token},
            )
            return check_response(conn)
