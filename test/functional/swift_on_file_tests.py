...
from test.functional.swift_test_client import Account, Connection, ResponseError
...
class TestSwiftOnFileEnv:
    ...
    def setUp(cls):
        ...
        for f in cls.files:
            ...
            file.write_random(
                cls.file_size, hdrs={"Content-Type": "application/octet-stream"}
            )
            stored_files.add(f)
...
class TestSwiftOnFile(Base):
    ...
    def test_container_listing_on_file_created_over_mountpoint(self):
        file_name = Utils.create_name()
        ...
        # Wait 2 seconds that watcher update container
        time.sleep(2)
        files = self.env.container.files()
        self.assert_status(200)
        self.assert_(file_name in files)
        ...
        files = self.env.container.files()
        self.assert_status(200)
        self.assert_(file_name not in files)
    ...
    def testObjectMetadataWhenFileModified(self):
        ...
        # We have to wait 2 seconds because mtime is for second
        time.sleep(2)
        new_data = b"I am Antman"
        new_data_hash = hashlib.md5(new_data).hexdigest()
        with open(file_path, "wb") as f:
            f.write(new_data)
        # Make sure GET works
